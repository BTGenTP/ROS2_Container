...
echo "[sim_env] Starting Xvfb on ${DISPLAY} (${VNC_RESOLUTION}x${VNC_COL_DEPTH})"
Xvfb "${DISPLAY}" -screen 0 "${VNC_RESOLUTION}x${VNC_COL_DEPTH}" -ac +extension GLX +render -noreset &

echo "[sim_env] Starting XFCE session"
dbus-launch --exit-with-session xfce4-session >/tmp/xfce.log 2>&1 &

echo "[sim_env] Starting x11vnc on port ${VNC_PORT}"
x11vnc -display "${DISPLAY}" -nopw -forever -shared -rfbport "${VNC_PORT}" -xkb >/tmp/x11vnc.log 2>&1 &

echo "[sim_env] noVNC available on :${NOVNC_PORT}"
echo "[sim_env] Workspace mounted at /workspaces/nav4rails"
